...
@login_required
def commande_ajouter(request):
    """Reçoit le form de menu_client.html et délègue à commandes app."""
    if request.method == 'POST':
        from commandes.views import commande_ajouter_client
        return commande_ajouter_client(request)
    return redirect('menu_client')


@login_required
def commande_recommander_redirect(request, cmd_id):
    """Re-commande depuis panier.html."""
    if request.method == 'POST':
        from commandes.views import commande_recommander
        return commande_recommander(request, cmd_id)
    return redirect('panier')
